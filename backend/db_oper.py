...
def get_db_connection(user_number: str, bot_number: str, db_name: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM CONNECTION WHERE user_number = ? and bot_number = ? ",
...
def new_db_connection(
    user_number: str, bot_number: str, result: str, db_name: str
) -> Literal["new entry created!"]:
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO CONNECTION (user_number, bot_number, value) VALUES (?,?,?)",
        (user_number, bot_number, result),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def update_db_connection(
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE CONNECTION SET VALUE = ? WHERE user_number = ? AND bot_number = ?",
        (result, user_number, bot_number),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def del_db_connection(
    user_number: str, bot_number: str, db_name: str
) -> Literal["data deleted!"]:
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
...
def create_participant(db_name: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE PARTICIPANT (GROUPID TEXT PRIMARY KEY, GROUPNAME TEXT)"
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def create_contacts(db_name: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE CONTACTS (CONTACT_ID TEXT PRIMARY KEY, CONTACT_NAME TEXT, PRIVATE_CHAT BOOL)"
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def insert_participants(participant: list[tuple], db_name: str):
    ...
    for i in participant:
        ...
        if not hasil:
            count += 1
            cursor.execute(
                "INSERT INTO PARTICIPANT (GROUPID, GROUPNAME) VALUES (?,?)",
                (i[0], i[1]),
            )
            conn.commit()
...
def set_private_chat(contacts: list[str], db_name: str):
    ...
    for i in contacts:
        cursor.execute(
            "UPDATE CONTACTS SET PRIVATE_CHAT = ? WHERE CONTACT_ID = ?", (True, i)
        )
    conn.commit()
    cursor.close()
    conn.close()
...
def insert_token_usage(
    user_number: str, timestamp: int, tokens: tuple[int, int, int], db_name: str
):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO TOKENS (USER_NUMBER, TIMESTAMP, PROMPT_TOKENS, COMPLETION_TOKENS, TOTAL_TOKENS) VALUES (?,?,?,?,?)",
        (user_number, timestamp, tokens[0], tokens[1], tokens[2]),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def insert_info_cs(user_number: str, timestamp: int, db_name: str):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO INFO_CS (USER_NUMBER, TIMESTAMP) VALUES (?,?)",
        (user_number, timestamp),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def get_conversation_messages(
    ...
    limit: int = 100,
    offset: int = 0,
    db_name: str = "backend/data/cipibot.db",
):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    if bot_number:
        cursor.execute(
            "SELECT * FROM CONVERSATION WHERE user_number = ? AND bot_number = ? ORDER BY id DESC LIMIT ? OFFSET ?",
            (user_number, bot_number, limit, offset),
        )
    else:
        cursor.execute(
            "SELECT * FROM CONVERSATION WHERE user_number = ? ORDER BY id DESC LIMIT ? OFFSET ?",
            (user_number, limit, offset),
        )
    result = cursor.fetchall()
    cursor.close()
    conn.close()
...
def get_all_conversations_sql(
    limit: int = 1000, offset: int = 0, db_name: str = "backend/data/cipibot.db"
):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
    cursor.execute(
        "SELECT * FROM CONVERSATION ORDER BY id DESC LIMIT ? OFFSET ?", (limit, offset)
    )
    result = cursor.fetchall()
    cursor.close()
    conn.close()
...
def get_conversation_count(db_name: str = "backend/data/cipibot.db"):
    conn = sqlite3.connect(db_name)
    cursor = conn.cursor()
